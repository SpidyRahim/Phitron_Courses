...
vector<int> merge_sort(vector<int> a, int k, int &count)
{
    ...
    for (int i = 0; i < a.size(); i++)
    {
        ...
        else
        {
            sorted_a.push_back(sorted_c[idx2]);
            idx2++;
            // check if the current element in the right subarray and the next element in the left subarray sum up to k
            if (idx1 < sorted_b.size() && sorted_b[idx1] + sorted_c[idx2 - 1] == k)
            {
                count++;
            }
        }
